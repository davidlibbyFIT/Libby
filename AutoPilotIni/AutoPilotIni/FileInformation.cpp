...
{
	refresh();
	m_strLastWrite=getStringLastWriteTime();
}
...
{	
	std::string currentLastWrite=getStringLastWriteTime();
	while(m_strLastWrite == currentLastWrite)
	{
		Sleep(msSleep);
		refresh();
		currentLastWrite = getStringLastWriteTime();
		return FILE_INFO_WAIT_PAUSE;
	}

	m_strLastWrite = currentLastWrite;

	return FILE_INFO_WAIT_DIFFRENCE;
}
