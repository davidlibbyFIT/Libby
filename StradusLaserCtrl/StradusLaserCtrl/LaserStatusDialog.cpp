...
LRESULT LaserStatusDialog::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	m_Button_Set=GetDlgItem(ID_APPLY);
	...
	std::string WindowText="Laser Dialog ( Not Found )";
	...
	if(!m_pLaser->isConnected())
	{
		m_Button_Start_Stop.EnableWindow(false);
		m_Edit_Power.EnableWindow(false);
		m_Button_Set.EnableWindow(false);
	
	}else
	{
		m_Button_Start_Stop.EnableWindow(true);
		m_Edit_Power.EnableWindow(true);
		m_Button_Set.EnableWindow(true);
		...
		std::string wave;
		if(m_pLaser->getLaserWavelength(wave)==STRADUS_OK)
		{
			WindowText="Laser Dialog ( "+wave+" nm )";
		}
	}
...
LRESULT LaserStatusDialog::OnBnClickedStartStop(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	//m_bLaserState=!m_bLaserState;
	

	if(m_pLaser->isConnected())
	{
		bool currentLaserState;
		if(m_pLaser->getEmissionStatus(currentLaserState)==STRADUS_OK)
		{
			bool LaserOn=true;
			...
			m_pLaser->setLaserEmission(LaserOn);
...
void LaserStatusDialog::UpdateLaserInfo()
{
	...
	float powermW=0;			
	if( m_pLaser->getLaserPower(powermW)==STRADUS_OK)
	{
		std::stringstream powerString;
		powerString<<(int)powermW;
		if(m_PastPowermW != powermW)
		{
			m_Edit_Power.SetWindowTextW(CA2W(powerString.str().c_str()).m_szBuffer);	
			m_PastPowermW=powermW;
		}
	}
	

	int TempC;
	if(m_pLaser->getBaseTempC(TempC)==STRADUS_OK)
	{		
		SetCurrentTempC((float)TempC);
	}

	bool emissionStatus=false;
	if(m_pLaser->getEmissionStatus(emissionStatus)==STRADUS_OK)
	{		
		if(emissionStatus)
		{
			m_bLaserState=3;
		...
		{
			bool Istate=false;
			m_pLaser->getInterlockStatus(Istate);
			if(Istate)
			{
				m_bLaserState=0;
			}
...
LRESULT LaserStatusDialog::OnBnClickedApply(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	std::string powerStr=RetriveStdStringFromCWindow(m_Edit_Power);
	
	int power=atoi(powerStr.c_str());
...
std::string LaserStatusDialog::RetriveStdStringFromCWindow(CWindow &TempWindow)
{
...
LRESULT LaserStatusDialog::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
